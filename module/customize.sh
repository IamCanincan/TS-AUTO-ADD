...
if [ "$user_count" -gt 0 ]; then
    ...
    if [ -s "$tmp_file" ]; then
        write_target_config "$tmp_file" && ui_print "  ✅ 已写入 $env_name 配置文件" || ui_print "  ⚠️ 写入失败 (可能 TeeSim 缺少 config.json)"
    else
        ui_print "  ⚠️ 未获取到有效应用列表，将在首次启动时处理"
    fi
    rm -f "$tmp_file" 2>/dev/null
else
    ui_print "  ⚠️ 当前无第三方应用，将在系统启动后自动同步"
fi
