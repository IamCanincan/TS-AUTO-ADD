...
[ -n "$INOTIFY_INFO" ] || abort "  ❌ 未检测到 inotifywait 或 inotifyd"
...
if [ "$TAA_BACKEND" = "teesim" ]; then
    [ -n "$AWK_CMD" ] || ui_print "  ⚠️ 未找到 awk，TEE Simulator 配置将无法维护"
...
case "$rc" in
    0) ui_print "  ✅ 写入完成" ;;
    1) ui_print "  ℹ️ 数据与现有配置一致" ;;
    2) ui_print "  ⚠️ 未能写入（已记入模块描述），交由守护进程处理" ;;
esac

ui_print "================================================"
ui_print "  ✅ 安装完成，重启后生效"
ui_print "  📝 运行日志：$TAA_LOG"
ui_print "================================================"
