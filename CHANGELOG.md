...
* **日志缓冲文件**：固定路径 `/data/adb/ts_auto.log`（权限 `600`、仅 root 可读、跨重启保留、卸载时自动删除），每行带 `月-日 时:分:秒` 时间戳；管理器的“操作”按钮优先展示缓冲内容，安装完成提示与模块日志中标注该路径。
...
* **日志改为双写（logcat + 本地缓冲）**：日志同时写 `logcat`（tag `TS-AUTO`）与 `/data/adb/ts_auto.log`，写入缓冲超过 64KB 时自动只保留末尾 200 行；缓冲不可写时静默退化为只写 `logcat`。原因是 `logcat` 为内存环形缓冲（默认几百 KB 且全系统共用），系统繁忙时旧记录很快被挤掉、重启即清空，事后排查往往已看不到。
